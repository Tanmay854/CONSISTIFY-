...
const LiquidGlassToggle = ({
  ...
  return (
    <button
      ...
      style={style}
      className={`relative w-11 h-11 rounded-full overflow-hidden backdrop-blur-xl bg-gradient-to-br from-[hsl(var(--foreground)/0.25)] to-[hsl(var(--foreground)/0.08)] ring-1 ring-foreground/20 shadow-[0_8px_24px_rgba(0,0,0,0.45)] flex items-center justify-center text-foreground active:scale-95 transition-transform duration-200 ${className}`}
    >
      {/* specular highlight */}
      <span className="pointer-events-none absolute -top-3 -left-2 w-10 h-10 rounded-full bg-[hsl(var(--foreground)/0.28)] blur-md" />
      {/* small glint */}
