...
const ReelsTab = ({ muted = false, feed: feedId = "quick_spark", active: paneActive = true }: { muted?: boolean; feed?: string; active?: boolean }) => {
  ...
  // Let Android WebView own the fling and CSS snap lifecycle. Forcing scrollTop
  // from a delayed touch/scroll handler can cancel a compositor-thread gesture
  // and makes swipes appear ignored on devices with different fling timings.
  const handleScroll = useCallback(() => {
    const el = containerRef.current;
    if (!el) return;
    const h = el.clientHeight;
    if (h <= 0) return;
    const next = Math.max(0, Math.min(feed.length - 1, Math.round(el.scrollTop / h)));
  ...
  return (
    <>
      <div
        ...
        className="h-[100dvh] overflow-y-scroll snap-y snap-mandatory scrollbar-hide overscroll-y-contain touch-pan-y [-webkit-overflow-scrolling:touch]"
      >
